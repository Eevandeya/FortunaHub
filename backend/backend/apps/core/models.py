...
class SaunaConfig(models.Model):
    ...
    def is_booking_within_open_hours(self, start_dt: dt.datetime, end_dt: dt.datetime) -> bool:
        one_day = dt.timedelta(days=1)

        if start_dt.time() < self.opening_time:
            opening_dt = timezone.make_aware(dt.datetime.combine(start_dt.date() - one_day, self.opening_time))
        else:
            opening_dt = timezone.make_aware(dt.datetime.combine(start_dt.date(), self.opening_time))

        if self.opening_time < self.closing_time:
            closing_dt = timezone.make_aware(dt.datetime.combine(opening_dt.date(), self.closing_time))
        else:
            closing_dt = timezone.make_aware(dt.datetime.combine(opening_dt.date() + one_day, self.closing_time))
