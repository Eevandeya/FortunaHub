...
const BookingProvider = ({children}) => {
    ...
    const actions = {
        ...
        setTimeSlot: (timeSlot) => dispatch({type: 'SET_TIME_SLOT', payload : timeSlot}),
        setVisitorsCount: (visitorsCount) => dispatch({type: 'SET_VISITORS_COUNT', payload : visitorsCount}),
        setPreferredContactMethod: (preferredContactMethod) => dispatch({type : 'SET_PREFERRED_CONTACT_METHOD', payload : preferredContactMethod}),
        resetBookings: () => dispatch({type : 'RESET_BOOKINGS'}),

        getBookingData: () => ({
          customer: state.customer,
          items: state.items,
          start_datetime: state.timeSlot?.start,
          end_datetime: state.timeSlot?.end,
